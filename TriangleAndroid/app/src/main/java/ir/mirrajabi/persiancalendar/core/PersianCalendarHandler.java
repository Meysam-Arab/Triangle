...
public class PersianCalendarHandler {
    ...
    public List<Day> getDays(int offset) {
        ...
        //meysam - added for setting underline for tasks
        ArrayList<TaskModel> tasks = new DatabaseHandler(mContext.getApplicationContext()).getTasksByYearAndMonth(String.valueOf(year), String.valueOf(month));
//        tasks.addAll(new DatabaseHandler(mContext.getApplicationContext()).getMonthlyTasksByMonth(String.valueOf(month)));
        ArrayList<Integer> dayOfMonths = new ArrayList<>();
        ArrayList<TaskModel> rozane = new ArrayList<>();
        ArrayList<Integer> haftegi = new ArrayList<>();
        for(int i = 0; i < tasks.size();i++)
        {
            if(tasks.get(i).getDayOfMonth() != null)
                dayOfMonths.add(Integer.valueOf(tasks.get(i).getDayOfMonth()));
            if(tasks.get(i).getType() == TaskModel.TYPE_DAILY)
                rozane.add(tasks.get(i));
            if(tasks.get(i).getType() == TaskModel.TYPE_WEEKLY)
                for (String dow : tasks.get(i).getDayOfWeek())
                {
                    haftegi.add(Integer.valueOf(dow));
                }
...
//                //meysam - set holiday color
//
//                //convert shamsi to miladi
                String miladi = Utility.convertDatePersian2Gorgeian(persianDate.getYear() + "/" + persianDate.getMonth() + "/" + persianDate.getDayOfMonth());
                ...
                todayHijri = todayHijri.plusDays(new SessionModel(mContext.getApplicationContext()).getIntegerItem(SessionModel.KEY_CALENDER_GHAMARI_CORRECTION, 0));
...
//                List<CalenderEventModel> monthCalenderEvents = new DatabaseHandler(mContext.getApplicationContext()).getCalenderEventsByMonthAndType(temp_arab[1],null);
                ArrayList<CalenderEventModel> calenderEvents = new DatabaseHandler(mContext.getApplicationContext()).getCalenderEventsByDateAndType(Utility.addZeroFirst(temp_arab[1], 2) + Utility.addZeroFirst(temp_arab[2], 2), CalenderEventModel.TYPE_GHAMARI);

                for(int j = 0; j < calenderEvents.size();j++)
                {
                    if(calenderEvents.get(j).getHoliday() == true)
                    {
                        day.setHoliday(true);
                        break;
                    }
...
//                ct.setGregorianDate(new Integer(temp[0]), new Integer(temp[1]), new Integer(temp[2]));
...
//                ArrayList<TaskModel> tasks = new DatabaseHandler(mContext.getApplicationContext()).getTasksByTime(String.valueOf(cal.getPersianYear()), Utility.addZeroFirst(String.valueOf(cal.getPersianMonth()),2),
//                        String.valueOf(Utility.getDayNumberOfWeek(cal.getPersianWeekDayName())),
//                        Utility.addZeroFirst(String.valueOf(cal.getPersianDay()),2), null, date);
//                if(tasks.size() > 0)
//                    day.setUnderLine(true);

                if(tasks.size() > 0)
                {
                    String todayPersianString = String.valueOf(day.getPersianDate().getYear())+Utility.addZeroFirst(String.valueOf(day.getPersianDate().getMonth()),2)+Utility.addZeroFirst(String.valueOf(day.getPersianDate().getDayOfMonth()),2);
                    Integer todayPersianInteger = Integer.valueOf(todayPersianString);

                    for (int j = 0; j < rozane.size(); j++) {
                        Integer startDateInteger = Integer.valueOf(rozane.get(j).getStartDate());
                        Integer endDateInteger = Integer.valueOf(rozane.get(j).getEndDate());


                        if(todayPersianInteger <= endDateInteger && todayPersianInteger >= startDateInteger)
                        {
                            day.setUnderLine(true);
                            break;
                        }
                    ...
                    if(dayOfMonths.contains(new Integer(i)))
                        day.setUnderLine(true);


                    if(haftegi.contains(day.getDayOfWeek()))
                        day.setUnderLine(true);
    ...
    public void clearLocalEvents(){ mLocalEvents.clear();}
